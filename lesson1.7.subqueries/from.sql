...
-- example 2.1.myVersion
SELECT CONCAT(customer_address_city_country.first_name,' ',customer_address_city_country.last_name) AS full_Name,
	customer_address_city_country.country,
	SUM(p.amount) AS total_amount
FROM payment p JOIN
	(
		SELECT c.customer_id, c.first_name, c.last_name, cn.country
		FROM customer c
			JOIN address a ON c.address_id = a.address_id
            JOIN city ct ON a.city_id = ct.city_id
            JOIN country cn ON ct.country_id = cn.country_id
    ) customer_address_city_country
    ON p.customer_id = customer_address_city_country.customer_id
GROUP BY customer_address_city_country.customer_id;
